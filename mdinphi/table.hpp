...
using get_forks_handler =
    std::function<void( size_t, fork_handler, fork_handler )>;
using release_forks_handler = std::function<void( size_t )>;

class table
{
public:
    table( size_t size, evilquinn::asio_context::handle asio );
    void get_forks( size_t pos,
                    fork_handler on_success,
                    fork_handler on_fail );
    void release_forks( size_t pos );

private:
    size_t on_the_right_of( size_t pos );
    void get_forked( size_t pos,
                     fork_handler on_success,
                     fork_handler on_fail );
