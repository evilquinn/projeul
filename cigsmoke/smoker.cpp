...
void evilquinn::cig_smokers::smoker::quit_smoking() { quit_smoking_ = true; }
...
void evilquinn::cig_smokers::smoker::smoke()
{
    ...
    smoke_timer_.async_wait( [this]( const boost::system::error_code& ec ) {
        if ( !ec )
        {
            ...
            boost::asio::post( *asio_, [this]() { wait(); } );
        }
    } );
}
...
std::string evilquinn::cig_smokers::smoker::to_string()
{
    ...
    oss << static_cast<int>( item_ ) << " smoked " << smoke_count_
        << " time, waited " << wait_count_ << " times.";
