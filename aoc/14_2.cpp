...
public:
    ...
    void validate_rules(const rules_type& rules)
    {
        for ( auto&& rule : rules )
        {
            for ( auto&& chem : rule.second.second )
            {
                if ( rules.count(chem.first) != 1 && chem.first != "ORE" )
                {
                    throw std::runtime_error(std::string("found chemical with no rule to build: " + chem.first));
                }
            }
        }
    }
    size_t calc_amount_for_ore(size_t quantity, const std::string& target_name)
    {
        size_t result = 0;
        if ( rules_.count(target_name) == 0 )
        {
        ...
        while ( ores <= quantity && ores > 0 )
        {
            ...
            while ( ! ( cauldron.size() == 1 && cauldron.count("ORE") == 1 ) )
            {
                ...
                {
                    for ( auto rule_chem : cauldron_target_rule.second )
                    {
                        cauldron[rule_chem.first] += rule_chem.second;
                    }
                    if ( cauldron_target_rule.first.second < target.second )
                    {
                        // not yet satisfied
                        target.second -= cauldron_target_rule.first.second;
                    }
                    else
                    {
...
int main()
{
    ...
    for ( auto&& datum : data )
    {
        ...
        nanofactory nf(ss);
        auto result = nf.calc_amount_for_ore(1000000000000, "FUEL");
